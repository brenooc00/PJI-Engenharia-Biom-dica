...
df_all <- cbind(df1_controle,df1_problema)
df_all_filter <- df_all |> select(-matches("^Tempo$"))

#Aplicando o shapiro.test() em todas as colunas do dataframe
teste_normalidade <- lapply(df_all_filter, shapiro.test)
...
#Se o p-value encontrado for maior do que 0.05, os dados seguem uma distribuição normal
#Se for menor, segue uma distribuição não-normal
...
if(length(normais) > 0){
  print("Grupos que seguem distribuição normal:")
  print(names(normais))
}

if(length(nao_normais) > 0){
  print("Grupos que seguem distribuição não normal:")
