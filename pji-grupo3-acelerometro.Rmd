...
fft_signal <- function(time, signal, name){
  ...
  df <- data.frame(ff = ff[1:(length(ff)/2)],mag = signal_mag[1:(length(signal_mag)/2)], theta = signal_theta[1:(length(signal_theta)/2)])
  
  plot_mag <- ggplot2::ggplot(df) + geom_line(aes(x = ff, y = mag), color = "green") + xlab("Frequência (Hz)") + ylab("Magnitude") + labs(title = paste("Espectro de amplitude do sinal", name))
...
fft_C1 <- fft_signal(df1_controle$Tempo, df1_controle$C1, "(grupo controle C1)")
fft_C2 <- fft_signal(df1_controle$Tempo, df1_controle$C2, "(grupo controle C2)")
fft_C3 <- fft_signal(df1_controle$Tempo, df1_controle$C3, "(grupo controle C3)")
fft_C4 <- fft_signal(df1_controle$Tempo, df1_controle$C4, "(grupo controle C4)")
fft_C5 <- fft_signal(df1_controle$Tempo, df1_controle$C5, "(grupo controle C5)")

#Amplitude do sinal grupo problema motor
fft_DP1 <- fft_signal(df1_problema$Tempo, df1_problema$DP1, "(grupo problema motor DP1)")
fft_DP2 <- fft_signal(df1_problema$Tempo, df1_problema$DP2, "(grupo problema motor DP2)")
fft_DP3 <- fft_signal(df1_problema$Tempo, df1_problema$DP3, "(grupo problema motor DP3)")
fft_DP4 <- fft_signal(df1_problema$Tempo, df1_problema$DP4, "(grupo problema motor DP4)")
fft_DP5 <- fft_signal(df1_problema$Tempo, df1_problema$DP5, "(grupo problema motor DP5)")
```
...
dif_quartil_C1 <- dif_quartil(fft_C1$mag, "C1 (grupo controle)")
dif_quartil_C2 <- dif_quartil(fft_C2$mag, "C2 (grupo controle)")
dif_quartil_C3 <- dif_quartil(fft_C3$mag, "C3 (grupo controle)")
dif_quartil_C4 <- dif_quartil(fft_C4$mag, "C4 (grupo controle)")
dif_quartil_C5 <- dif_quartil(fft_C5$mag, "C5 (grupo controle)")
print("")
dif_quartil_DP1 <- dif_quartil(fft_DP1$mag, "DP1 (grupo problema motor)")
dif_quartil_DP2 <- dif_quartil(fft_DP2$mag, "DP2 (grupo problema motor)")
dif_quartil_DP3 <- dif_quartil(fft_DP3$mag, "DP3 (grupo problema motor)")
dif_quartil_DP4 <- dif_quartil(fft_DP4$mag, "DP4 (grupo problema motor)")
dif_quartil_DP5 <- dif_quartil(fft_DP5$mag, "DP5 (grupo problema motor)")
```
