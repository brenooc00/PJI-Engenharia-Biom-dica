...
filtro <- function(signal, time, ordem, Fmin, Fmax, name){
  ...
  dygraph(df, main = paste("Gráfico grupo Controle", name))|>
    dyAxis("x", label = "Tempo (s)", labelHeight = 30) |>
    dyAxis("y", label = "Amplitude", labelWidth = 30) |>
...
df_filter_controle <- data.frame(lapply(names(df1_controle)[-1], function(col_name){
  filtro(df1_controle[[col_name]], df1_controle$Tempo, n, f1, f2, col_name)
}))
...
df_filter_problema <- data.frame(lapply(names(df1_problema)[-1], function(col_name){
  filtro(df1_problema[[col_name]], df1_problema$Tempo, n, f1, f2, col_name)
}))
...
df_fft_FiltControl <- data.frame(lapply(names(df_filter_controle)[-1], function(col_name){
  fft_signal(df1_problema[[col_name]], df1_problema$Tempo, n, f1, f2, col_name)
}))
